...
from .protocol import (
    NetworkConnection, Packet, PacketType, NetworkError,
    coordinate_to_tile, tile_to_coordinate
)
from .game_state import GameState, CompanyID, CompanyInfo, VehicleInfo, ClientInfo
from .commands import CommandPacket, CommandBuilder, Commands, DoCommandFlag
...
class OpenTTDClient:
    ...
    def __init__(self, server: str, port: int = 3979, player_name: str = "AI_Player",
                 company_name: str = "AI Co", password: str = "", company_password: str = "",
                 timeout: float = 30.0):
        """
    ...
    def get_our_company(self) -> Optional[Any]:
        ...
        if self.game_state.company_id is not None and self.game_state.company_id != 255:
            return self.game_state.companies.get(int(self.game_state.company_id))
        return None
    ...
    def get_our_vehicles(self) -> Dict[int, Any]:
        ...
        if self.game_state.company_id is None or self.game_state.company_id == 255:
            return {}
        from dataclasses import asdict
        return {vid: asdict(vehicle) 
                for vid, vehicle in self.game_state.vehicles.items() 
                if vehicle.owner == self.game_state.company_id}
    ...
    def get_map_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'has_map_data': hasattr(self.game_state.map_info, 'compressed_data') and self.game_state.map_info.compressed_data is not None
        }
    ...
    def send_command(self, command: CommandPacket) -> bool:
        ...
        try:
            ...
            self.connection.send_packet(packet)
            logger.info(f"Sent command: {command.command_id} (company={command.company}, p1={command.p1}, p2={command.p2}, text='{command.text}')")
            return True
    ...
    def build_rail(self, start: Tuple[int, int], end: Tuple[int, int], rail_type: int = 0) -> bool:
        """
        ...
        command = CommandBuilder.build_rail(start_tile, end_tile, rail_type, 
                                          int(self.game_state.company_id))
        return self.send_command(command)
    
    def build_train_depot(self, x: int, y: int, direction: int = 0, rail_type: int = 0) -> bool:
        """Build a train depot at coordinates"""
        tile = coordinate_to_tile(x, y, self.game_state.map_info.size_x)
        command = CommandBuilder.build_train_depot(tile, direction, rail_type,
                                                 int(self.game_state.company_id))
        return self.send_command(command)
    ...
    def build_train(self, depot_x: int, depot_y: int, engine_id: int = 0) -> bool:
        ...
        command = CommandBuilder.build_vehicle(depot_tile, engine_id, 
                                             int(self.game_state.company_id))
        return self.send_command(command)
    ...
    def create_company(self, company_name: str = "Bot Company", password: str = "") -> bool:
        ...
        logger.info(f"Company creation will happen automatically during join process: {company_name}")
    ...
    def build_railway_line(self, start: Tuple[int, int], end: Tuple[int, int], 
                          rail_type: int = 0) -> bool:
    ...
    def build_station_with_rail(self, x: int, y: int, direction: int = 0, 
                               platform_length: int = 4, num_platforms: int = 1) -> bool:
        ...
        try:
            ...
            logger.info(f"Building station at ({x}, {y}) with {num_platforms} platforms, length {platform_length}")
    ...
    def buy_and_start_train(self, depot_x: int, depot_y: int, engine_id: int = 0,
                           destination_station: Optional[Tuple[int, int]] = None) -> bool:
    ...
    def terraform_area(self, x: int, y: int, width: int, height: int, 
                      new_height: int) -> bool:
    ...
    def clear_area(self, x: int, y: int, width: int, height: int) -> bool:
        ...
        logger.info(f"Cleared {success_count}/{total_tiles} tiles in area ({x},{y}) to ({x+width},{y+height})")
    ...
    def set_company_colour(self, scheme: int = 0, primary: bool = True, colour: int = 0) -> bool:
        ...
        command = CommandBuilder.set_company_colour(scheme, primary, colour, int(self.game_state.company_id))
        success = self.send_command(command)
        if success:
            logger.info(f"🎨 Requested colour change: scheme={scheme}, primary={primary}, colour={colour}")
        return success
    ...
    def get_company_finances(self) -> dict:
        ...
        finances['debt_ratio'] = finances['loan'] / max(finances['value'], 1) if finances['value'] > 0 else 1.0
    ...
    def get_economic_status(self) -> dict:
        ...
        return {
            ...
            'inflation_construction': self.game_state.economy.inflation_rates.get('construction', 1.0),
            'interest_rate': self.game_state.economy.interest_rate,
            'max_loan_default': self.game_state.economy.max_loan
        }
    
    def calculate_loan_interest(self, loan_amount: int = None) -> int:
    ...
    def find_suitable_depot_location(self, near_x: int, near_y: int, 
    ...
    def find_best_route(self, start: Tuple[int, int], end: Tuple[int, int]) -> List[Tuple[int, int]]:
    ...
    def estimate_construction_cost(self, start: Tuple[int, int], end: Tuple[int, int],
                                 construction_type: str = "rail") -> int:
    ...
    def get_company_finances(self) -> Optional[Dict[str, Any]]:
        """Get detailed financial information for our company"""
        ...
        return {
            ...
            'expenses': getattr(company, 'expenses', 0),
            'net_worth': getattr(company, 'money', 0) - getattr(company, 'loan', 0),
            'company_value': getattr(company, 'company_value', 0)
        }
    ...
    def can_afford(self, estimated_cost: int) -> bool:
        ...
        return available_money >= estimated_cost * 1.2  # 20% safety margin
    ...
    def get_vehicle_statistics(self) -> Dict[str, Any]:
        ...
        for vehicle in our_vehicles.values():
            vtype = getattr(vehicle, 'vehicle_type', 'unknown')
            if vtype in stats['by_type']:
                stats['by_type'][vtype] += 1
            
            stats['total_profit'] += getattr(vehicle, 'profit_last_year', 0)
            stats['running_costs'] += getattr(vehicle, 'running_cost', 0)
    ...
    def auto_manage_company(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'recommendation': 'Consider expanding rail network' if finances and finances['money'] > 50000 else 'Focus on profitability'
        }
    ...
    def broadcast_status(self) -> bool:
        ...
        if company and finances:
            status_msg = (f"Bot Status: {info['vehicles']} vehicles, "
    ...
    def _handle_packet(self, packet: Packet) -> None:
        ...
        try:
            ...
            if hasattr(self, handler_name):
                ...
                logger.debug(f"No handler for packet type: {packet.type} (size: {len(packet_data)} bytes)")
        except Exception as e:
            logger.error(f"Error handling packet {packet.type}: {e}")
    ...
    def _send_join(self) -> None:
        ...
        logger.debug(f"CLIENT_JOIN: Requesting to join company_id={company_id_to_join} (COMPANY_NEW_COMPANY)")
    ...
    def _handle_server_join(self, packet: Packet) -> None:
        ...
        if joining_client_id == self.game_state.client_id:
            ...
            if self.game_state.company_id == CompanyID.COMPANY_NEW_COMPANY:
                logger.info(f"Sending automatic CMD_COMPANY_CTRL for company creation (current company_id: {self.game_state.company_id})")
    ...
    def _handle_server_command(self, packet: Packet) -> None:
        ...
        try:
            command_id = Commands(command_id_raw)
        except ValueError:
    ...
    def _handle_server_config_update(self, packet: Packet) -> None:
        ...
        try:
            ...
            logger.info(f"⚙️ Server config: max_companies={max_companies}, server_name='{server_name}'")
    ...
    def _handle_server_game_info(self, packet: Packet) -> None:
        ...
        try:
            ...
            if game_info_version >= 3:
                ...
                logger.info(f"REAL GAME DATE: Current year {current_year}, Started year {start_year}")
            ...
            logger.info(f"UPDATED GAME STATE - Year: {current_year}, Companies: {companies_on}/{companies_max}, Clients: {clients_on}/{clients_max}")
    ...
    def _handle_server_move(self, packet: Packet) -> None:
        ...
        try:
            ...
            if client_id == self.game_state.client_id:
                from .commands import CompanyID
                
                # Update our company state
                old_company_id = self.game_state.company_id
                self.game_state.company_id = company_id
