...
class EconomyInfo:
    ...
    inflation_rates: Dict[str, float] = field(default_factory=lambda: {
        'payment': 1.0,
        'construction': 1.0
    })
    max_loan: int = 500000
...
class GameState:
    ...
    def add_client(self, client: ClientInfo) -> None:
        ...
        if client.company_id != CompanyID.COMPANY_SPECTATOR and client.company_id not in self.companies:
